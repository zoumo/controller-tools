...
func (cw *codeWriter) rewriteImportAlias(f *jen.File) {
	for pkg, alias := range imports.ImportAlias() {
...
func (cw *codeWriter) GenerateScheme(metav1Pkg *loader.Package) error {
	...
	scheme := imports.Qual("k8s.io/client-go/kubernetes/scheme", "Scheme")
	schemefile.Func().Id("init").Params().BlockFunc(func(g *jen.Group) {
		g.Add(imports.Qual("k8s.io/apiextensions-apiserver/pkg/apis/apiextensions/v1beta1", "AddToScheme").Call(scheme.Clone()))
		for pkg := range cw.parser.GroupVersions {
			if pkg == metav1Pkg {
				continue
			}
			g.Add(imports.Qual(loader.NonVendorPath(pkg.PkgPath), "AddToScheme").Call(scheme.Clone()))
		}
	})

	cw.rewriteImportAlias(schemefile)
...
func (cw *codeWriter) GenerateCrds(crds map[string][]interface{}) error {
	...
	for version, list := range crds {
		...
		slice := jen.Index().Op("*").Add(imports.Qual("k8s.io/apiextensions-apiserver/pkg/apis/apiextensions/"+version, "CustomResourceDefinition"))
	...
	cw.rewriteImportAlias(crdsfile)
